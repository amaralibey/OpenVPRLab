...
DOWNLOAD_PATH="$PROJECT_ROOT/data/val/pitts30k-val"
...
echo "Saving to path: $DOWNLOAD_PATH/pitts30k-val.zip"

if ! curl -L -o "$DOWNLOAD_PATH/pitts30k-val.zip" "$DOWNLOAD_URL"; then
...
if ! unzip -q "$DOWNLOAD_PATH/pitts30k-val.zip" -d "$DOWNLOAD_PATH"; then
    error "Failed to unzip the file."
fi

# Remove the zip file after successful extraction
rm "$DOWNLOAD_PATH/pitts30k-val.zip" || echo "Warning: Failed to remove zip file: $DOWNLOAD_PATH/pitts30k-val.zip"
